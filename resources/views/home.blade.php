    ...
    <div class="row mt-2 g-1">
        ...
        <div class="col-md-3">
            <div class="card p-2 {{$job->job_type}}">
                <div class="text-right"> <small class="badge text-bg-info">{{$job->type}}</small> </div>
...
<div class="container">
    ...
    @foreach(\App\Models\job_posts::take(3)->orderBy('id','DESC')->get() as $job_post)
    <div class="card mb-3">
        <div class="card-body">
            <div class="d-flex flex-column flex-lg-row">
                ...
                <div class="row flex-fill">
                    <div class="col-sm-5">
                        <h4 class="h5">{{$job_post->title}}</h4>
                        <span class="badge bg-secondary">{{$job_post->job_type}}</span> <span class="badge bg-success">${{number_format($job_posts->salary,2)}}</span>
                    </div>
                    
                    <div class="col-sm-7 text-lg-end">
                        <a href="{{route('job.show',$job_post->slug)}}" class="btn btn-dark stretched-link">View</a>
                    </div>
                </div>
            </div>
        </div>
